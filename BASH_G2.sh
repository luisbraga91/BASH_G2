#!/bin/bash
#
# Script criado em 07/12/2016
#Autor Luis Filipe Matias Braga
# Este script tem o objetivo de disponibilizar formas de efetuar um backup de arquivos em computadores distintos, com os comandos Rsync ou Scp
...
define_software() {	# função da opção define_software
    ...
    read sBackupLuis		# efetua a leitura de entradas para á variável
echo "teste"
    clear		# limpa tela
    if test $sBackupLuis -eq 1	# inicio if
	then
	clear		# limpa tela
...
efetua_backup(){	# função da opção efetua_backup
	...
	scp -vP $portaSsh $nomeDiretorio/$nomeArquivo $usuarioBackup@$enderecoIp:$nomeCaminho	# funçao scp
    else
	clear	# limpa tela
	echo "Opção inválida."		# interação com usuário
